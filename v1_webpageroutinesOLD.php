...
function WebPageComposer_Popups () {
    ...
    XINTXTID("editabletextlinklink","editabletextlinklink","","50","100");XBR();
...
function Webpage_WEBPAGEPUBLISH_Output($webpage_name) {
	...
	$plugina = explode(",",$GLOBALS{'webpage_pluginlist'});
	foreach ($plugina as $pluginstring) {
	    // [ItemListA:Category=Event_All;Date=Future;SortBy=Date;SortSeq=Asc;Show=Full;Max=10;]
	    // XH1($pluginstring);
	    $wtype = String2PluginType($pluginstring);
	    $wparmvala = String2PluginParmVals($pluginstring);
	    
	    if ( $wtype == "ItemListA" ) { Plugin_ItemListA_Publish ($webpage_name,$wparmvala); }
	    // if ( $wtype == "ItemListB") { Plugin_ItemListB_Publish ($webpage_name,$wparmvala); }
	    // if ( $wtype == "SingleItemA") { Plugin_SingleItemA_Publish ($webpage_name,$wparmvala); }
	    if ( $wtype == "BulletinBoard" ) { Plugin_BulletinBoard_Publish ($webpage_name,$wparmvala); }
	    // if ( $wtype == "AdBanner") { Plugin_AdBanner_Publish ($webpage_name,$wparmvala); }
	    // if ( $wtype == "AdCarousel") { Plugin_AdCarousel_Publish ($webpage_name,$wparmvala); }
	    // if ( $wtype == "AdMultiple") { Plugin_AdMultiple_Publish ($webpage_name,$wparmvala); }
	    if ( $wtype == "GRLLeagueTableA") { Plugin_GRLLeagueTableA_Publish ($webpage_name,$wparmvala); }
	    if ( $wtype == "GRLLeagueTableB") { Plugin_GRLLeagueTableB_Publish ($webpage_name,$wparmvala); }
	    if ( $wtype == "GRLResults") { Plugin_GRLResults_Publish ($webpage_name,$wparmvala); }	    
	    if ( $wtype == "GRLResultsGrid") { Plugin_GRLResultsGrid_Publish ($webpage_name,$wparmvala); }
	    if ( $wtype == "GRLAttendance") { Plugin_GRLAttendance_Publish ($webpage_name,$wparmvala); }
	    if ( $wtype == "GRLFormTable") { Plugin_GRLFormTable_Publish ($webpage_name,$wparmvala); }
	    if ( $wtype == "GRLGoalsTable") { Plugin_GRLGoalsTable_Publish ($webpage_name,$wparmvala); }	    
	    if ( $wtype == "GRLFTLeagueTable") { Plugin_GRLFTLeagueTable_Publish ($webpage_name,$wparmvala); }
	    if ( $wtype == "GRLFTResults") { Plugin_GRLFTResults_Publish ($webpage_name,$wparmvala); }
	    if ( $wtype == "GRLFTFixtures") { Plugin_GRLFTFixtures_Publish ($webpage_name,$wparmvala); }
	}
...
function Webpage_DBHTML2PAGE_Output($webpage_html,$fullwidthenabled) {
	...
	while ( $hi < sizeof($htmlrawa) ) {
		...
		// <p id="fwparms_2" class="Plugin Tyyyymmddhhmmss" style="word-break: break-all;"><img src="../site_assets/Plugin_BulletinBoard.png" width="100%">[BulletinBoard:Name=Test;]</p>
		...
		/*
		if (strlen(strstr($htmlrawline,'class="BulletinBoard"')) > 0) {
		    $specialexpansion = "1";
		    $hi++; $htmlrawline = $htmlrawa[$hi]; // skip <p
		    $parmvala = String2PluginParmVals($htmlrawline);
		    //  <!-- PLUGIN [BulletinBoard:Name=Test;]--> <!-- /PLUGIN -->
		    array_push($htmla,'<!-- PLUGIN ['.String2PluginParmString($htmlrawline).'] -->');
		    $whtmla = BulletinBoardHTMLGen($parmvala);
		    $htmla = array_merge($htmla, $whtmla);
		    array_push($htmla,'<!-- /PLUGIN -->');
		    $hi++; // skip </p>
		}

		if (strlen(strstr($htmlrawline,'class="ItemListA"')) > 0) {
		    $specialexpansion = "1";
		    $hi++; $htmlrawline = $htmlrawa[$hi]; // skip <p
		    $parmvala = String2PluginParmVals($htmlrawline);
		    array_push($htmla,'<!-- PLUGIN ['.String2PluginParmString($htmlrawline).'] -->');
		    array_push($htmla, $wshtmla);
		    $whtmla = Array();
		    $itemcategorya = explode('_',$parmvala[0]);
		    if ($itemcategorya[0] == "Event") { $whtmla = EventListHTMLAGen($parmvala); }
		    if ($itemcategorya[0] == "Article") { $whtmla = ArticleListHTMLAGen($parmvala); }
		    if ($itemcategorya[0] == "Course") { $whtmla = CourseListHTMLAGen($parmvala); }
		    if ($itemcategorya[0] == "Draw") { $whtmla = DrawListHTMLAGen($parmvala); }
		    $htmla = array_merge($htmla, $whtmla);
		    array_push($htmla,'<!-- /PLUGIN -->');
		    $hi++; // skip </p>
		}
		
		if (strlen(strstr($htmlrawline,'class="GRLResults"')) > 0) {
		    $specialexpansion = "1";
		    $hi++; $htmlrawline = $htmlrawa[$hi]; // skip <p
		    $parmvala = String2PluginParmVals($htmlrawline);
		    array_push($htmla,'<!-- PLUGIN ['.String2PluginParmString($htmlrawline).'] -->');
		    $whtmla = GRLResultsHTMLAGen($parmvala);
		    $htmla = array_merge($htmla, $whtmla);
		    array_push($htmla,'<!-- /PLUGIN -->');
		    $hi++; // skip </p>
		}				
		
		if (strlen(strstr($htmlrawline,'class="GRLLeagueTableA"')) > 0) {
		    $specialexpansion = "1";
		    $hi++; $htmlrawline = $htmlrawa[$hi]; // skip <p
		    $parmvala = String2PluginParmVals($htmlrawline);
		    array_push($htmla,'<!-- PLUGIN ['.String2PluginParmString($htmlrawline).'] -->');
		    $whtmla = GRLLeagueTableAHTMLAGen($parmvala);
		    $htmla = array_merge($htmla, $whtmla);
		    array_push($htmla,'<!-- /PLUGIN -->');
		    $hi++; // skip </p>
		}
		
		if (strlen(strstr($htmlrawline,'class="GRLLeagueTableB"')) > 0) {
		    $specialexpansion = "1";
		    $hi++; $htmlrawline = $htmlrawa[$hi]; // skip <p
		    $parmvala = String2PluginParmVals($htmlrawline);
		    array_push($htmla,'<!-- PLUGIN ['.String2PluginParmString($htmlrawline).'] -->');
		    $whtmla = GRLLeagueTableBHTMLAGen($parmvala);
		    $htmla = array_merge($htmla, $whtmla);
		    array_push($htmla,'<!-- /PLUGIN -->');
		    $hi++; // skip </p>
		}
		
		if (strlen(strstr($htmlrawline,'class="GRLResultsGrid"')) > 0) {
		    $specialexpansion = "1";
		    $hi++; $htmlrawline = $htmlrawa[$hi]; // skip <p
		    $parmvala = String2PluginParmVals($htmlrawline);
		    array_push($htmla,'<!-- PLUGIN ['.String2PluginParmString($htmlrawline).'] -->');
		    $whtmla = GRLResultsGridHTMLAGen($parmvala);
		    $htmla = array_merge($htmla, $whtmla);
		    array_push($htmla,'<!-- /PLUGIN -->');
		    $hi++; // skip </p>
		}
		
		if (strlen(strstr($htmlrawline,'class="GRLAttendance"')) > 0) {
		    $specialexpansion = "1";
		    $hi++; $htmlrawline = $htmlrawa[$hi]; // skip <p
		    $parmvala = String2PluginParmVals($htmlrawline);
		    array_push($htmla,'<!-- PLUGIN ['.String2PluginParmString($htmlrawline).'] -->');
		    $whtmla = GRLAttendanceHTMLAGen($parmvala);
		    $htmla = array_merge($htmla, $whtmla);
		    array_push($htmla,'<!-- /PLUGIN -->');
		    $hi++; // skip </p>
		}
		
		if (strlen(strstr($htmlrawline,'class="GRLFormTable"')) > 0) {
		    $specialexpansion = "1";
		    $hi++; $htmlrawline = $htmlrawa[$hi]; // skip <p
		    $parmvala = String2PluginParmVals($htmlrawline);
		    array_push($htmla,'<!-- PLUGIN ['.String2PluginParmString($htmlrawline).'] -->');
		    $whtmla = GRLFormTableHTMLAGen($parmvala);
		    $htmla = array_merge($htmla, $whtmla);
		    array_push($htmla,'<!-- /PLUGIN -->');
		    $hi++; // skip </p>
		}
		
		if (strlen(strstr($htmlrawline,'class="GRLGoalsTable"')) > 0) {
		    $specialexpansion = "1";
		    $hi++; $htmlrawline = $htmlrawa[$hi]; // skip <p
		    $parmvala = String2PluginParmVals($htmlrawline);
		    array_push($htmla,'<!-- PLUGIN ['.String2PluginParmString($htmlrawline).'] -->');
		...
		if (strlen(strstr($htmlrawline,'class="GRLFTLeagueTable"')) > 0) {
		    $specialexpansion = "1";
		    $hi++; $htmlrawline = $htmlrawa[$hi]; // skip <p
		    $parmvala = String2PluginParmVals($htmlrawline);
		    array_push($htmla,'<!-- PLUGIN ['.String2PluginParmString($htmlrawline).'] -->');
		    $whtmla = GRLFTLeagueTableHTMLAGen($parmvala);
		    $htmla = array_merge($htmla, $whtmla);
		    array_push($htmla,'<!-- /PLUGIN -->');
		    $hi++; // skip </p>
		}
		
		if (strlen(strstr($htmlrawline,'class="GRLFTResults"')) > 0) {
		    $specialexpansion = "1";
		    $hi++; $htmlrawline = $htmlrawa[$hi]; // skip <p
		    $parmvala = String2PluginParmVals($htmlrawline);
		    array_push($htmla,'<!-- PLUGIN ['.String2PluginParmString($htmlrawline).'] -->');
		    $whtmla = GRLFTResultsHTMLAGen($parmvala);
		    $htmla = array_merge($htmla, $whtmla);
		    array_push($htmla,'<!-- /PLUGIN -->');
		    $hi++; // skip </p>
		}
		
		if (strlen(strstr($htmlrawline,'class="GRLFTFixtures"')) > 0) {
		    $specialexpansion = "1";
		    $hi++; $htmlrawline = $htmlrawa[$hi]; // skip <p
		    $parmvala = String2PluginParmVals($htmlrawline);
		    array_push($htmla,'<!-- PLUGIN ['.String2PluginParmString($htmlrawline).'] -->');
		    $whtmla = GRLFTFixturesHTMLAGen($parmvala);
		    $htmla = array_merge($htmla, $whtmla);
		    array_push($htmla,'<!-- /PLUGIN -->');
		    $hi++; // skip </p>
		}
		
		if (strlen(strstr($htmlrawline,'class="AdBanner"')) > 0) {
		    // <p id="fwparms_5” class=”AdBanner">
		    // <img src="../site_assets/Plugin_AdBanner.png" width="100%" />[AdBanner:Category=Class 2;]
		    // </p> 
		    $specialexpansion = "1";
		    $htmlrawline1 = $htmlrawline;
		    $hi++; $htmlrawline2 = '<'.$htmlrawa[$hi];
		    $parmvala = String2PluginParmVals($htmlrawline2);
		    array_push($htmla,'<!-- DPLUGIN ['.String2PluginParmString($htmlrawline2).'] -->');
		    array_push($htmla,$htmlrawline1);
		    $htmlrawline2 = str_replace("../site_assets",$GLOBALS{'site_wwwurl'}."/site_assets",$htmlrawline2);	
		    $htmlrawline2 = str_replace("Plugin_AdBanner.png","clearpixel.gif",$htmlrawline2);	
		    array_push($htmla,$htmlrawline2);
		    $hi++; $htmlrawline3 = '<'.$htmlrawa[$hi];
		    array_push($htmla,$htmlrawline3);		    
		    array_push($htmla,'<!-- /DPLUGIN -->');
		}
		*/
...
function Webpage_TEMPLATEPUBLISH_Output( $template_status,$template_name ) {
    ...
    array_push($cssa, '  background-color: #bbb; ');
    ...
    array_push($cssa, '  color: #fff;');
    array_push($cssa, '  background-color: #666;');
	...
	array_push($htmla, '<link href="'.$GLOBALS{'site_templateurl'}.'/font-awesome/css/font-awesome.min.css" rel="stylesheet" type="text/css">');
	array_push($htmla, YSITETEMPLATEJS('jquery'));
	array_push($htmla, YSITETEMPLATEJS('bootstrap'));
	// array_push($htmla, '<script type="text/javascript" src="'.$GLOBALS{'site_yuiurl'}.'/yahoo-dom-event/yahoo-dom-event.js"></script>');
	// array_push($htmla, '<script type="text/javascript" src="'.$GLOBALS{'site_yuiurl'}.'/container/container-min.js"></script>');
	// array_push($htmla, '<script type="text/javascript" src="'.$GLOBALS{'site_yuiurl'}.'/menu/menu.js"></script>');
	// array_push($htmla, '<script type="text/javascript" src="'.$GLOBALS{'site_yuiurl'}.'/connection/connection-min.js"></script>');
	...
	// array_push($htmla, '<link rel="stylesheet" type="text/css" href="'.$GLOBALS{'site_yuiurl'}.'/fonts/fonts-min.css" />');
	// array_push($htmla, '<link rel="stylesheet" type="text/css" href="'.$GLOBALS{'site_yuiurl'}.'/menu/assets/skins/sam/menu.css" />');
