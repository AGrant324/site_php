		...
		if (strlen(strstr($uploadcsva[0],"Data"))>0) {
			...
			foreach ($dataa as $dataelement) {
				if ( $headera[$hi] != "" ) {
					...
					if (strlen(strstr($fieldtypehash[$hbits[0]],"decimal"))>0) {					
						$dataelement = str_replace(',', '', $dataelement);					
						// XPTXT($headera[$hi]." ".$fieldtypehash[$headera[$hi]]);
					}
