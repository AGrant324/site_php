...
function Cor_SETUPCORPROGRAMME_Output() {
    ...
    $parm1 = $parm1."corprogramme_customtablist||||Yes|Custom Tabs|InputCheckboxFromList,FA2+FA3+FA4^"; 
...
function Cor_SETUPCORCURRENTUSE_Output() {
	$parm0 = "";
	$parm0 = $parm0."Current Use"."|"; # pagetitle
	$parm0 = $parm0."corcurrentuse"."|"; # primetable
	$parm0 = $parm0."|"; # othertables
	$parm0 = $parm0."corcurrentuse_id|"; # keyfieldname
	$parm0 = $parm0."corcurrentuse_id|"; # sortfieldname
	$parm0 = $parm0."25|"; # pagination
	$parm0 = $parm0."No"; # enable add-copy
	$parm1 = "";
	$parm1 = $parm1."corcurrentuse_id|Yes|Outlet Id|120|Yes|Outlet Id|KeyText,25,25^";
	$parm1 = $parm1."corcurrentuse_name|Yes|Outlet Name|120|Yes|Outlet Name|InputText,50,100^";
	$parm1 = $parm1."corcurrentuse_description|Yes|Outlet Description|120|Yes|Outlet Description|InputText,50,100^";	
	$parm1 = $parm1."generic_updatebutton|Yes|Update|70|No|Update|UpdateButton^";
	$parm1 = $parm1."generic_deletebutton|Yes|Delete|70|No|Delete|DeleteButton";
	GenericHandler_Output ($parm0,$parm1);
}
...
function Cor_CORSITEUPDATE_Output($corsite_id,$corsite_version,$thiscorsite_corprogramme,$currenttab) {
	...
	if (FoundInCommaList("FA4",$GLOBALS{'corprogramme_customtablist'})) { $tabviz["ASS1"] = Array("0","0","0","0","0","0"); }
	else { $tabviz["ASS1"] = Array("1","1","1","1","1","1"); }        
...
function LOCContentOutput() {
	...
	XBR();       
         if (FoundInCommaList("FA4",$GLOBALS{'corprogramme_customtablist'})) {
            BROW(); BCOLTXT("<b>Current Use:</b>","4"); BCOLINTEXTAREAID('corsite_currentuse','corsite_currentuse',$GLOBALS{'corsite_currentuse'},"2","6"); B_ROW();
            XBR();	
            BROW(); BCOLTXT("<b>Site Ref:</b>","4"); BCOLINTEXTAREAID('corsite_landref','corsite_landref',$GLOBALS{'corsite_landref'},"2","6"); B_ROW();
            XBR();
            BROW(); BCOLTXT("<b>Transfer Plan Ref:</b>","4"); BCOLINDATEID('corsite_transferplanref','corsite_transferplanref',$GLOBALS{'corsite_transferplanref'},"dd/mm/yyyy","6"); B_ROW();
            XBR();
            BROW(); BCOLTXT("<b>Area:</b>","4"); BCOLINDATEID('corsite_area','corsite_area',$GLOBALS{'corsite_area'},"dd/mm/yyyy","6"); B_ROW();
            XBR();	
            BROW(); BCOLTXT("<b>Locality:</b>","4"); BCOLINTXTID('corsite_locality','corsite_locality',$GLOBALS{'corsite_locality'},"6"); B_ROW();
            XBR();	
            BROW(); BCOLTXT("<b>Scheme:</b>","4"); BCOLINTXTID('corsite_scheme','corsite_scheme',$GLOBALS{'corsite_scheme'},"6"); B_ROW();
            XBR();
            BROW(); BCOLTXT("<b>Stock Grouping:</b>","4"); BCOLINTXTID('corsite_stockgrouping','corsite_stockgrouping',$GLOBALS{'corsite_stockgrouping'},"6"); B_ROW();
            XBR();
        } else {
...
function ASS2ContentOutput() {
    ...
    XDIV("SpecPh2View","");
    	...
    	if (($GLOBALS{'corsite_buildcomminternally'} == "Y")||($GLOBALS{'corsite_buildresiinternally'} == "Y")) {
    	    ...
    	    XBR();
            if ( FoundInCommaList("FA4",$GLOBALS{'corprogramme_customtablist'})) { $title = "<b>Site Assembly Costs:</b>"; } else { $title = "<b>Site Purchase Price:</b>"; }
    	    BROW(); BCOLTXT($title,"4"); BCOLINTXTIDCLASS('corsite_proposallandpurchasevalue','calcres','corsite_proposallandpurchasevalue',D80ToN80($GLOBALS{'corsite_proposallandpurchasevalue'}),"6"); B_ROW();
    	    XBR();
    	    ...
    	    XBR();
            if ( FoundInCommaList("FA4",$GLOBALS{'corprogramme_customtablist'})) { $title = "<b>Developer Profit:</b>"; } else { $title = "<b>Net Proceeds:</b>"; }
    	    BROW(); BCOLTXT($title,"4"); BCOLINTXTIDCLASS('corsite_proposalnetproceeds','calcres','corsite_proposalnetproceeds',D80ToN80($GLOBALS{'corsite_proposalnetproceeds'}),"6"); B_ROW();
            ...
            XBR();
            if ( FoundInCommaList("FA4",$GLOBALS{'corprogramme_customtablist'})) { $title = "<b>Land Value (net of planning costs):</b>"; } else { $title = "<b>Net Proceeds:</b>"; }
            BROW(); BCOLTXT($title,"4"); BCOLINTXTIDCLASS('corsite_proposalnetproceeds','calcres','corsite_proposalnetproceeds',D80ToN80($GLOBALS{'corsite_proposalnetproceeds'}),"6"); B_ROW();
            XBR();
            if ( FoundInCommaList("FA4",$GLOBALS{'corprogramme_customtablist'})) { $title = "<b>Land Value Uplift (from current use):</b>"; } else { $title = "<b>Uplift:</b>"; }
            BROW(); BCOLTXT($title,"4"); BCOLINTXTIDCLASS('corsite_proposaluplift','calcres','corsite_proposaluplift',$GLOBALS{'corsite_proposaluplift'},"6"); B_ROW();	
        }
    	B_COL();
    	BCOL("6");
    	...
    	XBR();
        if ( FoundInCommaList("FA4",$GLOBALS{'corprogramme_customtablist'})) { $title = "<b>Legal Costs (Site Assembly):</b>"; } else { $title = "<b>Legal Costs:</b>"; }
    	BROW();	BCOLTXT($title,"4"); BCOLINTXTIDCLASS('corsite_proposallegalcosts','calcres','corsite_proposallegalcosts',D80ToN80($GLOBALS{'corsite_proposallegalcosts'}),"6"); B_ROW();		
     	XBR();
...
function FA1ContentOutput() {
	...
	XBR();	
	BROW();
        if ( FoundInCommaList("FA4",$GLOBALS{'corprogramme_customtablist'})) { } else {
	...
	XDIV("InternalBuildDiv","");
		...
		XBR();
		BROW();
                if ( FoundInCommaList("FA4",$GLOBALS{'corprogramme_customtablist'})) { $title = "<b>Site Assembly Costs:</b>"; } else { $title = "<b>Land Purchase:</b>"; }
		BCOLTXT($title,"2");
		BCOLTXT(" ","4");
		...
		BCOLTXTCOLOR("&nbsp;","7","#f2d30c","black");
                if ( FoundInCommaList("FA4",$GLOBALS{'corprogramme_customtablist'})) { $title = "<b>Internal Developer Profit</b>"; } else { $title = "<b>Internal Build Net Proceeds</b>"; }
		BCOLTXTCOLOR($title,"2","#f2d30c","black");
		BCOLBACKCOLOR("#f2d30c","1");
