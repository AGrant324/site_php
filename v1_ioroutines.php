...
function Upload_File ($uploadname,$fpath,$filename,$aft,$fmaxsize,$aureqd,$tempprefix,$prefix,$maxwidth)  {
	...
	if ($_FILES[$uploadname]["name"] == "") {
		...
		XPTXTCOLOR($filename." uploaded. filesize ".$filesize." max ".$fmaxsize,"green");
...
// ====== firtly get the parameters returned by slim (actual image file is returned outside of slim as a base64 encoded string)
...
// ====== now get the actual input image as a file upload outside of slimas a base64 encoded string
if ($eichanged == "1") {
	if ($filename == "") { $filename = $_FILES[$uploadname]["name"]; }
	$filesize = $_FILES[$uploadname]["size"];
	$mbits = explode(".", $filename);
...
function Upload_SlimBase64Crop ($uploadname,$eichanged,$fpath,$filename,$aft,$fmaxsize,$aureqd,$tempprefix,$prefix,$filesuffix,$reqdwidth,$reqdheight)  {
    // uploadname = Input Field Name
    // eichanged = Existing Image Changed
    // fpath = Output File Path
    // filename = If File Name is to be forced
    ...
    $copycanvasstring = $_REQUEST["CopyCanvasString"];
    $copycanvaswidth = $_REQUEST["CopyCanvasWidth"];
    $copycanvasheight = $_REQUEST["CopyCanvasHeight"];

    /*
    XH2('SlimBase64Crop Parameters.');
    XTXT("uploadname - ".$uploadname);
    XBR();XTXT("eichanged - ".$eichanged);
    XBR();XTXT("fpath - ".$fpath);
    XBR();XTXT("filename - ".$filename);
    XBR();XTXT("aft - ".$aft);
    XBR();XTXT("fmaxsize - ".$fmaxsize);
    XBR();XTXT("aureqd - ".$aureqd);
    XBR();XTXT("tempprefix - ".$tempprefix);
    XBR();XTXT("prefix - ".$prefix);
    XBR();XTXT("filesuffix - ".$filesuffix);
    XBR();XTXT("reqdwidth - ".$reqdwidth);
    XBR();XTXT("reqdheight - ".$reqdheight);
    */
    ...
    $inimagesize = $image['input']['size']; // slim cropper image size
    $inimagewidth = $image['input']['width']; // slim cropper image size
    $inimageheight = $image['input']['height']; // slim cropper image size
    // {"server":null,"meta":{"userId":"1234"},"input":{"name":"Ascot2.jpg","type":"image/jpeg","size":189472,"width":1600,"height":1200,"field":"slim_input_2"},"actions":{"rotation":180,"crop":{"x":0,"y":100,"width":1600,"height":1000,"type":"manual"},"size":null}}
    $slimstring = $_REQUEST['slim'][0]; // Slim::getImages() does not return rotation info !
    $sbits = explode('"rotation":',$slimstring);
    $tbits = explode(',',$sbits[1]);
    $actionrotation = floatval($tbits[0]);  // null if no rotation, 90 180 270 anticlockwise
    $actioncropx = $image['actions']['crop']['x']; // slim cropper crop x
    $actioncropy = $image['actions']['crop']['y']; // slim cropper crop y
    $actioncropwidth = $image['actions']['crop']['width']; // slim cropper crop width
    $actioncropheight = $image['actions']['crop']['height']; // slim cropper crop height
    $actioncroptype = $image['actions']['crop']['type']; // auto?
    $actionsize = $image['actions']['size']; // null?

    $canvascropx = $actioncropx * $copycanvaswidth / $inimagewidth;
    $canvascropy = $actioncropy * $copycanvasheight / $inimageheight;
    $canvascropwidth = $actioncropwidth * $copycanvaswidth / $inimagewidth;
    $canvascropheight = $actioncropheight * $copycanvasheight / $inimageheight;
    ...
    XBR();XTXT("actioncroptype - ".$actioncroptype);
    XBR();XTXT("actionsize - ".$actionsize);
    XBR();XTXT("canvascropx - ".$canvascropx);
    XBR();XTXT("canvascropy - ".$canvascropy);
    XBR();XTXT("canvascropwidth - ".$canvascropwidth);
    XBR();XTXT("canvascropheight - ".$canvascropheight);
    */

    // ====== now get the actual input image as a file upload outside of slimas a base64 encoded string
    if ($eichanged == "1") {
        if ($filename == "") { $filename = $inimagename; }
        $filesize = strlen($copycanvasstring);  // redundant - file not actually uploaded fullsize
        $mbits = explode(".", $filename);
        $filetype = end($mbits);
        $filefirstname = str_replace(".".$filetype, "", $filename);
        // remove all blanks and commas in image firstname
        ...
        $data = explode( ',', $copycanvasstring );
        if (strlen(strstr($data[0],"image/png"))>0) { $filetype = "png"; } // workaround if gif canvas defaulted to png
        ...
        if ($filesize <= $fmaxsize) {
            if ((strlen(strstr($aft,$filetype))>0)||($aft == "all")) {
                if ($copycanvasstring == "") {
                    $errorcode = "1";
                    $message = "Return Code: Empty Canvas";
                } else {
                    // list($width, $height) = getimagesize($_FILES[$uploadname]["tmp_name"]);
                    if (file_exists($fpath."/".$filename)) {$GLOBALS{'IOWARNING'} = "1";}
                    $extramessage = "";
                    // if (($reqdwidth != "flex" )&&($width > $reqdwidth)) { $extramessage = ". Image width reduced to ".$reqdwidth." px"; }

                    if ($GLOBALS{'IOWARNING'} == "1") {
                        if ($aureqd == "") {
                            // create file from base64 string
                            $ifp = fopen( $fpath."/".$outname, 'wb' );
                            $data = explode( ',', $copycanvasstring );
                            fwrite( $ifp, base64_decode( $data[ 1 ] ) );
                            fclose( $ifp );
                            if ($actionrotation != "") { RotateImage( $fpath."/".$outname, $actionrotation); }
                            CropAndResizeImage( $fpath."/".$outname, $canvascropx, $canvascropy, $canvascropwidth, $canvascropheight, $reqdwidth, $reqdheight);
                            chmod($fpath."/".$outname, 0644);
                            $message = "Successfully updated file - ".$reqdwidth." ".$inimagename.$extramessage;
                            $autaken = "updated";
                        }
                        if ($aureqd == "update") {
                            // create file from base64 string
                            ...
                            CropAndResizeImage( $fpath."/".$outname, $canvascropx, $canvascropy, $canvascropwidth, $canvascropheight, $reqdwidth, $reqdheight);
                            chmod($fpath."/".$outname, 0644);
                            $message = "Successfully updated file - ".$inimagename.$extramessage;
                            $autaken = "updated";
                        }
                        if ($aureqd == "add") {
                            $errorcode = "1";
                            $message = "ERROR: File already exists - ".$inimagename;
                        }
                    }
                    if ($GLOBALS{'IOWARNING'} == "0") {
                        if ($aureqd == "") {
                            ...
                            CropAndResizeImage( $fpath."/".$outname, $canvascropx, $canvascropy, $canvascropwidth, $canvascropheight, $reqdwidth, $reqdheight);
                            chmod($fpath."/".$outname, 0644);
                            $message = "Successfully added file - ".$inimagename.$extramessage;
                            $autaken = "added";
                        }
                        if ($aureqd == "update") {
                            $errorcode = "1";
                            $message = "ERROR: File does not already exist - ".$inimagename;
                        }
                        if ($aureqd == "add") {
                            // create file from base64 string
                            $ifp = fopen( $fpath."/".$outname, 'wb' );
        ...
        if ($existingimagename == "") {
            ...
            fclose( $ifp );
            if ($actionrotation != "") { RotateImage( $fpath."/".$outname, $actionrotation); }
            CropAndResizeImage( $fpath."/".$existingimagename, $canvascropx, $canvascropy, $canvascropwidth, $canvascropheight, $reqdwidth, $reqdheight);
            $message = "Successfully updated existing image - ".$existingimagename.$extramessage;
            $autaken = "updated";
        }
    }
    ...
    // Return string - Error(1/0)|Message|outfilename|added/updated|filesize|width|height
    return "$errorcode|$message|$outname|$autaken|$filesize|$width|$height|$actionrotation";
}
...
function RotateImage( $imagefullname, $rotationangle) {
    // XH4("RotateImage ".$imagefullname."|".$rotationangle);
    $error = "0";
    $message = "";
    if (file_exists($imagefullname)) {
        $bits = explode("/",$imagefullname);
        ...
        */
        ...
        $rotated_image = imagerotate($current_image, $rotationangle, 0);

        if ($imagetype == "jpg") {imagejpeg($rotated_image, $imagefullname);}
        if ($imagetype == "jpeg") {imagejpeg($rotated_image, $imagefullname);}
        if ($imagetype == "gif") {imagegif($rotated_image, $imagefullname);}
        if ($imagetype == "png") {imagepng($rotated_image, $imagefullname);}
        if ($imagetype == "JPG") {imagejpeg($rotated_image, $imagefullname);}
        if ($imagetype == "JPEG") {imagejpeg($rotated_image, $imagefullname);}
        if ($imagetype == "GIF") {imagegif($rotated_image, $imagefullname);}
        if ($imagetype == "PNG") {imagepng($rotated_image, $imagefullname);}
...
function Get_Array_SelectX () {
	# datatype keys - returns array
        /* ==== original code ========
        $tableName = func_get_arg(0);
	$filter = func_get_arg(1);
	$return = func_get_arg(2);
	for ($i=0; $i < sizeof($filter); $i+=2) {
		$filterList .= $filter[$i]."='".$filter[$i+1]."'";
		if ($i < sizeof($filter)-2) {
			$filterList .= " AND ";
		}else{
			$filterList .= ";";
		}
	...
	if ($filter != "") {
		$query = "SELECT " . $return . " FROM " . $tableName . " WHERE " . $filterList;
	}else{
		$query = "SELECT " . $return . " FROM " . $tableName;
	}
        */

        // ==== new code ========
        $tableName = func_get_arg(0);
	$filtername = func_get_arg(1);
        $filtervalue = func_get_arg(2);
	...
	// echo $query;
	// echo "<br>";
	$GLOBALS{'IOERRORcode'} = "IO001"; $GLOBALS{'IOERRORmessage'} = "$query";
	$r = mysqli_query($GLOBALS{'IOSQL'},$query);
	// print_r($r);
	$tarray = array();
	if (mysqli_num_rows($r) > 0) {
	...
	// print "Get_Array_Select returns:<br>";
	// print "Get_Array_Select returns:<br>"; print_r($tarray); print "<BR>\n";
...
function Get_Array_Select2TEST () {
	XBR();
    # datatype keys - returns array
    $tableName = func_get_arg(0);
    $filter = func_get_arg(1);
...
function ADD_TABLE_NAME($table,$names){
	for ($i=0; $i < sizeof($names); $i++) {
		$names[$i] = $table."_".$names[$i];
	}
	return $names;
}
...
$qk1= "SELECT * FROM ".$tablename." WHERE ".$tfields[0]."='".$rootkey."'"; # forces domainid to be used
// $qk2 = ""; $qk3 = ""; $qk4 = "";
// if ($parmsmax > 0) {$qk2 = " AND ".$parms[0]."='".$parms[1]."'";}
// if ($parmsmax > 1) {$qk3 = " AND ".$parms[2]."='".$parms[3]."'";}
if ($parmsmax > 0) {$qk2 = " AND ".$tfields[1]."='".$parms[1]."'";}
if ($parmsmax > 1) {$qk3 = " AND ".$tfields[2]."='".$parms[2]."'";}
if ($parmsmax > 2) {$qk4 = " AND ".$tfields[3]."='".$parms[3]."'";}
$q = $qk1.$qk2.$qk3.$qk4;
	...
	while (($uploadcsv = fgetcsv($handle, 0, ",")) !== FALSE) {
		...
		if ($uploadcsv[0] == "dataheader") {
          ...
          XDIV("reportdiv_".$uploadtable,"container");
			...
			$tbits = explode("|",$uploadcsv[1]);
			foreach ($tbits as $tbit) {
			    $q = 'SHOW COLUMNS FROM '.$tbit;
    		    $r = mysqli_query($GLOBALS{'IOSQL'},$q);
    		    if (mysqli_num_rows($r) > 0) {
    		        while ($row = mysqli_fetch_array($r, MYSQLI_NUM)) {
				...
				for ($ui = 2; $ui < sizeof($uploadcsvheader); $ui++) {
				    ...
				    if ($uploadcsvheader[$ui] != "") { // updates requested
    					...
    					if (strlen(strstr($allfields,$thisfieldname))>0) {
    					    ...
    					    if ($GLOBALS{$thisfieldname."_sqldatatype"} == 'date') {
    					        if (strlen(strstr($thisfieldval,'/'))>0) {
    					            $GLOBALS{$thisfieldname} = DDbMMbYYYYtoYYYYhMMhDD($thisfieldval);
    					            array_push($newdataa, DDbMMbYYYYtoYYYYhMMhDD($thisfieldval));
    					            array_push($errordataa, "0");
    					            array_push($infodataa, "1");
    					            $reformatmessage = ' (INFO: Dates have been reformatted)';
    					        } else {
    					            $GLOBALS{$thisfieldname} = $thisfieldval;
    					            array_push($newdataa, $thisfieldval);
    					            array_push($errordataa, "0");
    					            array_push($infodataa, "0");
    					        }
    					    } else {
    					        $GLOBALS{$thisfieldname} = $thisfieldval;
    					        array_push($newdataa, $thisfieldval);
				...
				if ( $extendedtrace == "Yes" ) {
				    ...
				    for ($di = 0; $di < sizeof($newdataa); $di++) {
				        if ($uploadcsvheader[$di+2] != "") {
				            if ($errordataa[$di] == "0") {
				                if ($infodataa[$di] == "1") {
				                    XTDTXTCOLOR($newdataa[$di],"orange");
				                } else {
				    ...
				    if ($reformatmessage != "") { $reformatmessagecol = '<span style="color:orange";>'.$reformatmessage.'</span>'; }
				    else { $reformatmessagecol = ""; }
				    if ( $uploaderrormsg == "" ) {
				        if ( $uploadwarningmsg == "" ) {
				            XTDTXT('<span style="color:#3679B7">'.$uploaderrormsg.$uploadwarningmsg.$uploadactiontaken.'</span>'.$reformatmessagecol);
				        }
				        else {
				            XTDTXT('<span style="color:orange";>'.$uploaderrormsg.$uploadwarningmsg.$uploadactiontaken.'</span>'.$reformatmessagecol);
				        }
				    }
				    else {
				        XTDTXT('<span style="color:red";>'.$uploaderrormsg.$uploadwarningmsg.$uploadactiontaken.'</span>'.$reformatmessagecol);
				    }
				    X_TR();
				    if ($dataexists == "1") {
    				    XTR();
				    ...
				    if ( $uploaderrormsg == "" ) {
				        if ( $uploadwarningmsg == "" ) { XTDTXTCOLOR($uploaderrormsg.$uploadwarningmsg.$uploadactiontaken.$reformatmessage,"#3679B7"); }
				        else { XTDTXT($uploaderrormsg.$uploadwarningmsg.$uploadactiontaken.$reformatmessage); }
				    }
				    else { XTDTXTCOLOR($uploaderrormsg.$uploadwarningmsg.$uploadactiontaken.$reformatmessage,"red"); }
				    X_TR();
				}
			}
		}
...
function AddToMassUpdateLog($mulog, $fieldname, $oldvalue, $newvalue) {
    ...
    // XPTXTCOLOR($mulog."@".$fieldname."@".$oldvalue."@".$newvalue."@".$newmulog,"blue");
    return $newmulog;
}
...
function GET_COLUMN_NAMES($tablename){
    $carray = Array("table_name='".$tablename."'");
    return	$tableNames = Get_Array_Select2("information_schema.columns",$carray,"COLUMN_NAME");
}

function Get_Email($userID){
	$constraint = ['person_id = "'.$userID.'"'];
	return Get_Array_Select2("person",$constraint,"person_email1")[0][0];
}
?>
