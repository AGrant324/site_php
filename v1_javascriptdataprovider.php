...
# Conditions
# [singlekey=val1] - single records with this key
...
# OUTPUT
# tablex_keysï¿½2ï¿½ - Number of keys in table
# tablex_rootkeyï¿½rootkeyvalueï¿½ - Rootkey
# tablex_headerï¿½field1headerï¿½field2headerï¿½field3headerï¿½ - FieldNames returned
# tablex_dataï¿½field1valueï¿½field2valueï¿½field3valueï¿½ - FieldValues returned
...
# DataRequestList = table1[singlekey=val1],table2,table3 - single key for table 1
...
# Output Records = table3_dataï¿½field1ï¿½field2ï¿½field3ï¿½ -  
$datarequestlist = $_REQUEST["DataRequestList"];
#ï¿½print $datarequestlist;
#ï¿½$traceinfocaptured = "1";
#ï¿½$traceinfostring = $datarequestlist."ï¿½";
...
foreach ($datarequestlista as $datarequestlistelement) {
 ...
 if (strlen(strstr($datarequestlistelement,"["))>0) { # conditions have been specified
  ...
  foreach ($conditiona as $tcondition) {  
   ...
   if (strlen(strstr($tcondition,"singlekey="))>0) {
    # [singlekey=val1] - record with this key
    $singlekeyrequired = "1";
    $normalaccessrequired = "0";
    $afields = explode('=', $tcondition);
    $singlekeyvalue = $afields[1]; 	   
   }
 ...
 if ($singlekeyrequired == "1") { $datakeya = Array($singlekeyvalue); } 
 if ($rootkeyrequired == "1") { 
 ...
 foreach ($datakeya as $datakey) {
  ...
  if ($singlekeyrequired == "1") { 
        Get_Data($dt,$singlekeyvalue); 
  }    
  if ($rootkeyrequired == "1") { 
      if (strlen(strstr($rootkeyvalue,$rootkeyseparator)) > 0) {
        // multiple root keys
...
if ($traceinfocaptured == "1") {     print "trace_infoï¿½".$traceinfostring.$recsep; 
