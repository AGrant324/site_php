...
if ($inaccredupdate == "Response") {
    ...
    $GLOBALS{'accredcriteria_lastselfassessmentpersonid'} = $GLOBALS{'LOGIN_person_id'};
    $GLOBALS{'accredcriteria_lastselfassessmenttimestamp'} = $GLOBALS{'currenttimestamp'};
    Write_Data("accredcriteria",$inaccredscheme_id,$inaccredcriteria_clubid,$inaccredcriteria_id);
    ...
    while (($more == "1" )&&($its < 99)) {
     ...
     if (strlen(strstr($GLOBALS{'LOGIN_service_id'},"sfm"))>0) {
        // Create button to update rectifications 
        XFORMUPLOAD("sfmgroundselfassessmentin.php","sfmgroundselfassessmentform");
        XINSTDHID();
        XINHID("sfmclub_id",$inaccredcriteria_clubid);
        ...
        BCOLTXT("","9");
        BCOL("2");
        BINBUTTONIDSPECIAL("Save","primary","Save");
