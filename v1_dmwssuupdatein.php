...
if (($insubmitaction == "Save")||($insubmitaction == "SaveClose")) {
	...
	foreach ( $_REQUEST as $keystring => $v ) {
		...
		if ($keybits[0] == "dmwswellbeing") {
			...
			if ($fieldname == 'dmwswellbeing_score'){
			    if ($GLOBALS{'dmwswellbeing_score'} != 0) {
    			    $thisprogida = Get_Array('dmwswellbeing',$thisdmwssu_id);
    		        if (count($thisprogida) == 0){
    		            $GLOBALS{'dmwssu_initialwellbeingscore'} = $GLOBALS{'dmwswellbeing_score'}; // first wellbeing score
    		        } else {
    		            if ( $thisdmwsvisit_id == $thisprogida[0]) {
    		                $GLOBALS{'dmwssu_initialwellbeingscore'} = $GLOBALS{'dmwswellbeing_score'}; // update of first wellbeing score
    		            } else {
    		                if ( $thisdmwsvisit_id >= end($thisprogida) ) { // update of last visit or new visit
			...
			if (($keybits[1] == "endfield")&&($GLOBALS{'dmwswellbeing_score'} > 0)) {			
		...
		if ($keybits[0] == "dmwsprogress") {
			if ($keybits[1] == "startfield") {		    
			    ...
			    if ($GLOBALS{'IOWARNING'} == "1") {
			        ...
			        if (($GLOBALS{'dmwsprogress_treatmentnotes'} != null)||($GLOBALS{'dmwsprogress_healthnotes'} != null)||($GLOBALS{'dmwsprogress_wellbeingnotes'} != null)||($GLOBALS{'dmwsprogress_familynotes'} != null)||($GLOBALS{'dmwsprogress_relationshipsnotes'} != null)||($GLOBALS{'dmwsprogress_housingnotes'} != null)||($GLOBALS{'dmwsprogress_financenotes'} != null)||($GLOBALS{'dmwsprogress_worknotes'} != null)||($GLOBALS{'dmwsprogress_socialnotes'} != null)||($GLOBALS{'dmwsprogress_activitiesnotes'} != null)){
			            $record = "1";
			        }
			        else{$record = "0";}
			    }				
			...
			if ($fieldname == 'dmwsprogress_score'){
			    $thisprogida = Get_Array('dmwsprogress',$thisdmwssu_id);
			    if (count($thisprogida) == 0){
			        $GLOBALS{'dmwssu_initialprogressscore'} = $GLOBALS{'dmwsprogress_score'};
			    }
               
			    else{$GLOBALS{'dmwssu_finalprogressscore'} = $GLOBALS{'dmwsprogress_score'};}
			...
			if (($keybits[1] == "endfield")&&(($GLOBALS{'dmwsprogress_score'} > 0)||($record == "1"))) {
	...
	*/
        ...
        $GLOBALS{'dmwssu_casestatus'} = "";
        // XPTXT('Case Status Before |'.$GLOBALS{'dmwssu_casecloseddate'}."|".$GLOBALS{'dmwssu_casestatus'});
        // set case status - same as dmwsarchive
        if (($GLOBALS{'dmwssu_casecloseddate'} == '0000-00-00')||($GLOBALS{'dmwssu_casecloseddate'} == '')||($GLOBALS{'dmwssu_casecloseddate'} == '--')) {
            $GLOBALS{'dmwssu_casestatus'} = "open";
        } else {
            if (DaysDifference($GLOBALS{'currentYYYY-MM-DD'},$GLOBALS{'dmwssu_casecloseddate'}) > 395) { 
                $GLOBALS{'dmwssu_casestatus'} = "archived";
            } else {
                $GLOBALS{'dmwssu_casestatus'} = "closed";
            }
        }
	// XPTXT('Case Status After |'.$GLOBALS{'dmwssu_casecloseddate'}."|".$GLOBALS{'dmwssu_casestatus'});
