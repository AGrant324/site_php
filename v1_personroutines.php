...
function Person_MYMEMBERSHIP_Output () {
	...
	foreach (Get_Array('person') as $tpersonid ) {
		...
		#ï¿½XPTXT($tpersonid." ". $theiremaillist);
		if (MatchLists($myemaillist, $theiremaillist)  ==  true) {
			...
			#ï¿½XH5("MATCHED".$GLOBALS{'person_fname'}." ".$GLOBALS{'person_sname'});
    	}   
	}
	...
	if ($GLOBALS{'person_paidperiodid'} == $GLOBALS{'currperiodid'}) {
		...
		XTABLE();
		#ï¿½XH4("Select your payment method.");
		#ï¿½$xhash = List2Hash("One Off payment,Staged Payment");
		#ï¿½XINRADIOHASH($xhash,"payment_method",$GLOBALS{'person_type'});
		#ï¿½XBR();
		X_TABLE();
		XINSUBMIT("Next");
		X_FORM();
...
function Person_MEMBERSHIPANALYSIS_Output() {
	...
	#ï¿½$parm1 = $parm1."person_paidincrementamount|No|Amount|50|Yes|Paid Increment Amount|InputText,20,40^";
	#ï¿½$parm1 = $parm1."person_paidincrementnumber|No|Amount|50|Yes|Paid Increment Number|InputText,20,40^";		
...
function Person_NEWREG_Output ($mode,$person_id,$actioncode,$existingpersonidlist) {
	...
	if (VSF("personsafeguarding")) {
		...
		#ï¿½XTR();XTDTXT("Parent/Guardian Initials");XTDINTXT("person_parentmidinits",$GLOBALS{'person_parentmidinits'},"25","50");X_TR();
...
$mfparm = $mfparm."person_postcode|INTXT|person_postcode_label|INTXT|person_postcode|Yes|^";
if (VSF("personmedical")) { 
    $mfparm = $mfparm."person_medicalcondition|INRADIO|person_medicalcondition_label|INRADIO|person_medicalcondition|Yes|^"; 
    $mfparm = $mfparm."person_medicalallergy|INRADIO|person_medicalallergy_label|INRADIO|person_medicalallergy|Yes|^"; 
    $mfparm = $mfparm."person_medicalmedication|INRADIO|person_medicalmedication_label|INRADIO|person_medicalmedication|Yes|^";    
    $mfparm = $mfparm."person_emergencytel|INTEL|person_emergencytel_label|INTEL|person_emergencytel|Yes|^"; 
    $mfparm = $mfparm."person_doctor|INTXT|person_doctor_label|INTXT|person_doctor|Yes|^"; 
}
if (VSF("personmembership")) {	
    $mfparm = $mfparm."person_skills|INTXT|person_skills_label|INTXT|person_skills|Yes|^";
    $mfparm = $mfparm."person_willingtohelp|INTXT|person_willingtohelp_label|INTXT|person_willingtohelp|Yes|^";
}
if (VSF("personsafeguarding")) { 
    $mfparm = $mfparm."person_dob|INDATE|person_dob_label|INDATE|person_dob|Yes|^"; 
}
if (VSF("personethnicity")) { 
    $mfparm = $mfparm."person_ethnicity|INSELECT|person_ethnicity_label|INSELECT|person_ethnicity|Yes|^"; 
}
if (VSF("persondisability")) { 
    $mfparm = $mfparm."person_disability|INSELECT|person_disability_label|INSELECT|person_disability|Yes|"; 
}   
...
if (VSF("personsafeguarding")) {
	...
	#ï¿½XTR();XTDTXT("Other Parent/Guardian Initials");XTDINTXT("person_parent2midinits",$GLOBALS{'person_parent2midinits'},"25","50");X_TR();
...
function Person_DELETESEARCH_Output () {
	...
	# $parm2 = Buttons Id ï¿½ To,To..,ToDistList,ToPersonList,70|Cc,CC..,CcDistList,CcPersonList,70 or View,View..,personLUin.php,,70
...
$selectedperson = "0";
#ï¿½XH4($GLOBALS{'person_authority'});
...
foreach (Get_Array_Hash_SortSelect("team",$GLOBALS{'currperiodid'},"team_seq","","") as $tteam_code) {
	$selectedteam = "0";
	 ...
	 if (strlen(strstr($GLOBALS{'person_authority'},"RM#SectionRM="))>0) { $selectedteam = "1"; } #ï¿½CHECK - TO LOOSE
	 if (strlen(strstr($GLOBALS{'person_authority'},"RM#SectionLeader="))>0) { $selectedteam = "1"; }  #ï¿½CHECK - TO LOOSE
...
function Person_Action2Globals($tactionstring) {
	...
	foreach ( $tactionstringa as $tactionstringelement ) {
		...
		#ï¿½XPTXT($tactionstringelementa[0]." - ".$tactionstringelementa[1]);
	}
}
...
function UpdateRegisterList($listfieldname,$sessiondate,$personid,$parametername,$parametervalue) {
	#ï¿½XH5($listfieldname." ".$personid." ".$parametername." ".$parametervalue);	
	XH5("INPUT = ".$GLOBALS{$listfieldname});	
	// format  date|personid,attended,paid|personid,attended,paid^date|personid,attended,paid|personid,attended,paid  
	$list = $GLOBALS{$listfieldname};
	$listdh = Array();
	$listph = Array();
	#ï¿½XH5('existinglist '.$list);
	if ($list != "") {
		$listda = explode('^',$list);
...
function DeleteFromRegisterList($listfieldname,$sessiondate,$personid) {
	#ï¿½XH5($listfieldname." ".$personid." ".$parametername." ".$parametervalue);	
	#ï¿½XH5("INPUT = ".$GLOBALS{$listfieldname});	
	// format  date|personid,attended,paid|personid,attended,paid^date|personid,attended,paid|personid,attended,paid  
	$list = $GLOBALS{$listfieldname};
	$listdh = Array();
	$listph = Array();
...
function DeleteSessionFromRegisterList($listfieldname,$sessiondate) {
	#ï¿½XH5($listfieldname." ".$sessiondate);	
	#ï¿½XH5("INPUT = ".$GLOBALS{$listfieldname});	
	// format  date|personid,attended,paid|personid,attended,paid^date|personid,attended,paid|personid,attended,paid  
	$list = $GLOBALS{$listfieldname};
	$listdh = Array();
...
function Person_NEWREG_Output ($mode,$person_id,$actioncode,$existingpersonidlist) {
	...
	if ($mode == "REG") { 
		...
		X_TR();		
		XTR();XTDTXTID("regwarningmessage","");
		XTD();XINSUBMITID("RegFormSubmit","Send");X_TD();
		X_TR();
		X_TABLE();
	}
	if ($mode == "CONFIRM") {
		...
		XTABLE();
		XINHID("person_membershiptermsconsentdate",$GLOBALS{'currentYYYY-MM-DD'}); 
		XTR();XTDTXTID("confirmwarningmessage","");XTD();XINSUBMITID("ConfirmFormSubmit","Send");X_TD();X_TR();
		X_TABLE();
	}
	X_FORM();
	XBR();XBR();
...
if (VSF("personmembership")) {
	...
	X_TR();
	XTR();XTDTXTID("warningmessage","");
	XTD();XINSUBMITID("FormSubmit","Send");X_TD();
	X_TR();
	X_TABLE();
	X_FORM();
	XBR();XBR();
...
function Person_MYPREFERENCES_Output() {
    ...
    $parm1 = $parm1."person_id|Yes|Id|90|Yes|Id|KeyText,5,15^";
    $parm1 = $parm1."person_fname|Yes|FirstName|150|Yes|FirstName|Text^";
    $parm1 = $parm1."person_sname|Yes|SurName|150|Yes|SurName|Text,20,50^";
    $parm1 = $parm1."person_navigationpref|Yes|Navigation Alignment|60|Yes|Navigation Alignment|InputSelectFromList,Right+Left^";
    $parm1 = $parm1."generic_updatebutton|Yes|Update|70|No|Update|UpdateButton";
    GenericHandler_Output ($parm0,$parm1);
}
