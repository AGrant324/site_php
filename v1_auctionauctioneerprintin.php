...
$downloadfilename = $GLOBALS{'site_filepath'}."/".$GLOBALS{'LOGIN_domain_id'}."/auctioneerprint.rtf";
...
foreach ($lotarray as $lotarrayelement) {	
 ...
 fwrite($handle, $title.'\b0  '.$description." ".'\b - '.$donation." ".$share." ".$reserve.'\b0'.'\line'."\n");   
 fwrite($handle, '\line'."\n");
 $pagecount++;
 if ($pagecount > 9 ) { fwrite($handle, '\page'."\n"); $pagecount = 0; } 	
